...
Harbinger supports two agent backends for running jobs. Choose the one that fits your setup.
...
Run `harbinger setup` to reconfigure, or set the `AGENT_BACKEND` GitHub variable directly:

```bash
# Switch to Claude Code
npx harbinger set-var AGENT_BACKEND claude-code

# Switch to Pi
npx harbinger set-var AGENT_BACKEND pi
```
