...
| `DATABASE_PATH` | Override SQLite database location (default: `data/harbinger.sqlite`) | No |
| `HARBINGER_VERSION` | Package version for Docker image tags (auto-set by setup) | No |
...
Set via `npx harbinger set-agent-secret` and `npx harbinger set-agent-llm-secret`. Each value is stored exactly as provided.
...
| `JOB_IMAGE_URL` | Docker image path for job agent (e.g., `ghcr.io/myorg/mybot`) | No | `harbinger-ai/harbinger:job-${HARBINGER_VERSION}` |
| `EVENT_HANDLER_IMAGE_URL` | Docker image path for event handler | No | `harbinger-ai/harbinger:event-handler-${HARBINGER_VERSION}` |
