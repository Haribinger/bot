...
Harbinger has two layers that use LLMs independently:
...
```bash
npx harbinger set-var LLM_PROVIDER openai
npx harbinger set-var LLM_MODEL gpt-4o
```

The matching API key must exist as a GitHub secret:

```bash
npx harbinger set-agent-secret OPENAI_API_KEY sk-...
```
...
```bash
npx harbinger set-var LLM_PROVIDER custom
npx harbinger set-var LLM_MODEL qwen3:8b
npx harbinger set-var OPENAI_BASE_URL http://host.docker.internal:11434/v1
npx harbinger set-var RUNS_ON self-hosted
```
...
```bash
npx harbinger set-var LLM_PROVIDER custom
npx harbinger set-var LLM_MODEL deepseek-chat
npx harbinger set-var OPENAI_BASE_URL https://api.deepseek.com/v1
```

Then set the API key as a GitHub secret:

```bash
npx harbinger set-agent-secret CUSTOM_API_KEY sk-...
```
...
| Default job model | GitHub repo variables | `LLM_PROVIDER`, `LLM_MODEL` (set via `npx harbinger set-var`) |
